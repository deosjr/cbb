...
func generateMap(seed int64) (*cbb.TileMap, map[cbb.Coord]Terrain) {
	...
	for y := 0; y < mapH; y++ {
		for x := 0; x < mapW; x++ {
			...
			if dist < threshold {
				t := Plains
				// Forest in the interior (~35% of land)
				if dist < maxRadius*0.55 && rng.Float64() < 0.35 {
					t = Forest
				}
				terrain[c] = t
...
// findNearestTerrain returns the closest tile of type t to from, searching the full terrain map.
func findNearestTerrain(aw *annoWorld, from cbb.Coord, t Terrain) (cbb.Coord, bool) {
	best := cbb.Coord{}
	bestDist := math.MaxFloat64
	found := false
	for c, terrain := range aw.terrain {
		if terrain != t {
			continue
		}
...
// findNearestFishingSpot returns the closest passable tile that is adjacent to water.
func findNearestFishingSpot(aw *annoWorld, from cbb.Coord) (cbb.Coord, bool) {
	best := cbb.Coord{}
	bestDist := math.MaxFloat64
	found := false
	for c, terrain := range aw.terrain {
		...
		hasWater := false
		for _, adj := range adjacentCoords(c) {
			if aw.terrain[adj] == Water {
				hasWater = true
				break
			}
		}
